-- Example Dummy Data Script
-- NOTE: In order to test this, you must have actual user UUIDs generated by Supabase Auth.
-- Replace 'REPLACE_WITH_USER_UUID' with an actual user ID from the `auth.users` table.

-- Insert some default categories
-- Make sure a user exists before running this. We'll use a placeholder variable.
DO $$ 
DECLARE
  v_user_id UUID := 'REPLACE_WITH_USER_UUID'; -- Replace this with a real UUID string
  v_income_cat_id UUID;
  v_expense_cat_id UUID;
BEGIN
    -- Check if user exists before attempting insert (this is normally handled by the application)
